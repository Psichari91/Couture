...
class manualPairingWindow(QtWidgets.QDialog):
    def __init__(self, folderPath, coutureCore):
        ...
        self.licence_label = QtWidgets.QLabel("Couture is distributed under MIT License")
        self.licence_label.setAlignment(qc.Qt.AlignCenter)
        self.verticalLayout.addWidget(self.licence_label)

        self.MIT_pix = qg.QPixmap("%s/icons/MIT.png" % folderPath)
        self.MITButton = QtWidgets.QPushButton()
        self.MITButton.setIcon(self.MIT_pix)
        self.MITButton.setIconSize(qc.QSize(400, 141))
        self.MITButton.setMinimumHeight(141)
        self.MITButton.setStyleSheet("border: none;")

        self.MITButton.clicked.connect(partial(data.goToWeb,'https://opensource.org/licenses/MIT',None))
        self.verticalLayout.addWidget(self.MITButton)
